...
DOCKER_CONTAINER_NAME="kimjihoo_hccmovement"
BASE_PORT=8271
...
docker run -it --rm --device=nvidia.com/gpu=all --shm-size 1TB \
    ...
    ${DOCKER_IMAGE_NAME} \
    bash -c "
        echo '========== [STEP 1] Jupyter Notebook 실행 ==========' && \
        jupyter notebook --ip=0.0.0.0 --port=${RANDOM_PORT} --no-browser --allow-root --NotebookApp.token='' & \
        sleep 15 && \
        echo '========== [STEP 2] batch_pose_inference.py 실행 ==========' && \
        cd /workspace/nas203/ds_RehabilitationMedicineData/IDs/Kimjihoo/3_project_HCCmove/ipynb && \
        python3 batch_pose_inference.py
    "